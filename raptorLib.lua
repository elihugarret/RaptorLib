...
------------------------------------------------------------------------------------------
--Cargar Samples del Directorio.
local dir = "/home/elihu/Desktop/Samples/"
...
function rpattern(patrones,i,tono,dino,dur,vol1,vol2)
 ...
 for w, beat in ipairs(patrones[i]) do
       ...
       if beat == 'x' then  proAudio.soundPlay(kic,vol1,vol2)
   elseif beat == 'o' then proAudio.soundPlay(sna,vol1,vol2)  
   elseif beat == '*' then proAudio.soundPlay(hat,vol1,vol2)
   elseif beat == '-' then proAudio.soundPlay(ope,vol1,vol2)
   elseif beat == '&' then proAudio.soundPlay(rob,vol1,vol2)
   elseif beat == '#' then proAudio.soundPlay(vel,vol1,vol2)
   elseif beat == '+' then proAudio.soundPlay(iro,vol1,vol2)
   elseif beat == '$' then proAudio.soundPlay(exh,vol1,vol2)
   elseif beat == '@' then proAudio.soundPlay(air,vol1,vol2)
   elseif beat == '%' then proAudio.soundPlay(ice,vol1,vol2)
   elseif beat == '~' then proAudio.soundPlay(kit,vol1,vol2)
   elseif beat == '|' then proAudio.soundPlay(met,vol1,vol2)
   elseif beat == ';' then proAudio.soundPlay(tin,vol1,vol2)
...
for q = 0, arg.lenght do
  ...
  for i = 0, arg.every do
    ...
    if i == arg.every then raptor:seq({arg.variation},arg.scale2+q*(arg.shift1),
                                    arg.ugenVar,tempo(arg.tempo),arg.volR,arg.volL) 
     elseif i < arg.every then raptor:seq({arg.pattern},arg.scale1+q*(arg.shift2),
...
function eoraptor:swt(s)
 ...
 if e == s.every then raptor:swt(s.variations,s.indexVar,s.scale+(w*s.shift1),s.ugenVar,tempo(s.tempo),s.volR,s.volL)
 elseif e < s.every then raptor:swt(s.patterns,s.index,s.scale+(w*s.shift2),s.ugen,tempo(s.tempo),s.volR,s.volL)
   end
       end
     end
  end
------------------------------------------------------
function eoraptor:rrnd(r)
...
for t = 0, r.lenght do 
 ...
 for y = 0, r.every do
 if y == r.every then raptor:rrnd(r.variations,r.scale+(t*r.shift1),r.ugenVar,tempo(r.tempo),r.volR,r.volL)
 elseif y < r.every then raptor:rrnd(r.patterns,r.scale+(t*r.shift2),r.ugen,tempo(r.tempo),r.volR,r.volL) end  
   end
 end
end
--------------------------------------------------------------------
function eoraptor:splay(u)
